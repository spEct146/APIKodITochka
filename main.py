...
def ask_gpt(promt: str)->str:
    responce = g4f.ChatCompletion.create(
        model = "gpt-3.5-turbo", # модель нейросети 
        ...
        # по каким пунктам нейросеть оценивает диалог
    )
    return responce
# вместо '--' надо внести число от 1 до 13 (01, 02, . . . , 12, 13)
with open(f"00--.txt", 'r', encoding="utf8") as file:
    text = file.read()
